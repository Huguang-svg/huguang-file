...
APPID = '7718de03'  # 替换为你的APPID
APIKEY = 'e6173d51efef69ef13d8049573c67533'  # 替换为你的APIKey
APISECRET = 'Njc4NDViNWY4Y2ViOWRjNjUyYjA2MTEy'  # 替换为你的APISecret
...
class Ws_Param(object):
    def __init__(self, APPID, APIKey, APISecret, Text):
        ...
        self.BusinessArgs = {
            "tts": {
                "vcn": "x5_lingfeizhe_flow", # 语音合成模型，可参考 https://www.xfyun.cn/services/smart-tts
                "volume": 50, # 音量
                "rhy": 1, # 韵律
                "speed": 50, # 语速pi
                "pitch": 50, # 语调
...
def play_audio(file_path):
    ...
    try:
        if HAS_PYGAME:
            ...
            # 使用 playsound 库直接播放，不会弹出播放器
            playsound(file_path, block=True)
        else:
            # 最后的备选方案：使用系统命令（可能会弹出播放器）
            abs_path = os.path.abspath(file_path)
            system = platform.system()
            if system == "Windows":
                # 使用 PowerShell 的 Start-Process 静默播放（需要安装音频解码器）
                # 或者使用 winsound（仅支持 WAV）
                try:
                    import subprocess
                    # 尝试使用 ffplay（如果安装了 ffmpeg）
                    subprocess.run(['ffplay', '-nodisp', '-autoexit', abs_path], 
                                 capture_output=True, check=False)
                except:
                    # 如果都不可用，只能使用默认播放器（会弹出窗口）
